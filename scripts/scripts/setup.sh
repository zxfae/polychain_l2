...
check_requirements() {
    ...
    if command_exists dfx; then
        DFX_INSTALLED_VERSION=$(dfx --version 2>/dev/null | grep -oP '\d+\.\d+\.\d+' || echo "unknown")
...
install_dfx() {
    ...
    log_info "Attempting to install DFX $DFX_VERSION using install.sh..."
    export TERM=dumb
    curl -fsSL https://internetcomputer.org/install.sh | sh -s -- --yes --version "$DFX_VERSION" > install.log 2>&1 || {
        cat install.log
        log_warning "Official install script failed. Attempting fallback installation..."
        fallback_install_dfx
    }
    ...
    if command_exists dfx; then
        DFX_INSTALLED_VERSION=$(dfx --version 2>/dev/null | grep -oP '\d+\.\d+\.\d+' || echo "unknown")
        if [ "$DFX_INSTALLED_VERSION" = "$DFX_VERSION" ]; then
            log_success "DFX $DFX_VERSION installed successfully"
        else
            log_error "DFX version mismatch. Expected $DFX_VERSION, found $DFX_INSTALLED_VERSION"
            exit 1
        fi
    else
        log_error "DFX installation failed. No dfx binary found."
        exit 1
    fi
}

# Fallback DFX installation
fallback_install_dfx() {
    log_info "Downloading DFX $DFX_VERSION binary directly..."
    mkdir -p ~/bin
    curl -fsSL https://github.com/dfinity/sdk/releases/download/$DFX_VERSION/dfx-$DFX_VERSION-x86_64-linux.tar.gz | tar -xz -C ~/bin || {
        log_error "Failed to download or extract DFX $DFX_VERSION"
        exit 1
    }
    chmod +x ~/bin/dfx
    log_success "DFX $DFX_VERSION installed to ~/bin/dfx"
}
